...
<p>Важно отметить, что помимо набора параметров, есть такие атрибуты склада и такие пожелания пользователя, которые трудно учесть при простом математическом сравнении того, насколько сходятся параметры запроса и склада. В число таких атрибутов входит, например, стоимость услуг: каждый складской провайдер рассчитывает стоимость за услуги индивидуально, нет единого калькулятора. Помимо этого трудно учесть оценки других клиентов об услугах складского провайдера, график работы склада. Клиент может оставить дополнительный комментарий при создании запроса - любой текст с пожеланием, какой ему нужен склад. Эти параметры можно учесть только с помощью ручной оценки релевантности склада запросу.</p>
<p>Именно поэтому был необходим обучаемый алгоритм - он позволил выдавать рекомендации на основе реальной оценки человека.</p>
...
<p>Ниже представлено схематичное описание процессов обучения и использования рекомендательного алгоритма. Также описан код, который отвечает за работу системы.</p>
...
<p>Столбец с эвклидовым расстоянием в обучающей выборке нормализуется, чтобы все значения были в диапазоне от нуля до единицы. Затем рассчитывается взвешенное среднее арифметическое между нормализованным расстоянием и коэффициентом Жаккара. Таким образом получается примерная оценка релевантности склада запросу. Выборка выгружается в файл rating.csv</p>

<p>После этого я вручную откорректировал оценки релевантности, исходя из того, какие склады действительно подходят по запросу, какие были выбраны.</p>

<p>Получившуюся обучающая выборка передается на обучение модели.</p>

## training.py

<p>Для обучения подготавливаются embedding-слои склада и запроса. Формируется алфавит из токенов запроса, алфавит из id складов, чтобы отделить каждый из этих уникальных объектов.</p>
<p>В модели запроса есть атрибуты query_features (токенизированный набор параметров запроса), wh_longitude, wh_latitude (желаемая долгота и широта местоположения склада). В модели склада только атрибут warehouse_id (ID склада). Это делается, чтобы похожим запросам выдавались похожие рекомендации. В общей модели объединяются модели склада и запроса и в нее передаются оценки релевантности.</p>
<p>Каждый из атрибутов embedding-слоя запроса кодируется набором из 64 чисел. Чем больше чисел выбрано для кодирования, тем "различимее" будут между собой два запроса. Атрибуты склада кодируются 32 числами. Кодирование в числах используется, так как модели удобнее обучаться на числах, чем на тексте.</p>
<p>Модель обучается минимизировать функцию потери в выданной оценке релевантности. То есть решается задача регрессии. В качестве метрики ошибки я выбрал среднеквадратическое отклонение.</p>
<p>Итоговая модель была обучена на 250 эпохах с learning rate значением 0.000001.</p>

<p>После обучения модель экспортируется. Обученная модель находится в директории export.</p>

## connect.py

<p>После обучения и экспорта модели, она в формате SavedModel может загружаться в любой момент и использоваться для выдачи оценок. За это отвечает файл connect.py. Это Flask-приложение, которое принимает запросы с фронтенда, когда пользователь проходит опрос. Полученные данные приводятся в вид, нужный для модели (точно так же токенизируются все параметры запроса и передаются широта и долгота).</p>
<p>Затем отсекаются склады, которые находятся дальше, чем на 300 км от желаемого места. ID складов, которые остались передаются в модель для получения оценки релевантности запроса.</p>
<p>Отсортированный список с релевантностью и ID складов возвращается в формате JSON и получается фронтендом.</p>
