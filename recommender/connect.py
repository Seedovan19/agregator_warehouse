...
# Затем производится загрузка обученной, сохраненной заранее модели и она выдает значения "релевантности" складов запросу.
...
def get_recommendations():
    ...
    for wh_id in warehouse_ids:
        results[i][0] = wh_id
        # В модель для получения оценки склада передаются следующие данные:
        # 1. строка с атрибутами запроса, которые клиент, хочет чтобы были у склада. 
        #       Например, такая строка может выглядеть так: "palletization, pharmacy, crossdock"
        # 2. координаты желаемого расположения склада
        # 3. id складов, находящихся ближе чем на 300км к желаемой точке
        results[i][1] = (loaded({
